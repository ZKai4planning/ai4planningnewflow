...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const { data, error: apiError } = await sendOtp({ email: email.trim(), intent: 'login' })
      if (apiError || !data?.success) { setError(apiError ?? 'Something went wrong.'); return }

      const params = new URLSearchParams({ email: email.trim(), intent: 'login', returnTo })
      router.push(`/verify-email?${params.toString()}`)
    })
  }
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        ...
        <Button type="submit" variant="primary" size="lg" fullWidth isLoading={isPending} disabled={!email || isPending}>
          {isPending ? 'Sending code…' : 'Send sign-in code →'}
        </Button>

        <p className="text-center text-[12px] text-[#5A6A8A]">
          <Link href={`/forgot-password${email ? `?email=${encodeURIComponent(email)}` : ''}`} className="hover:text-[#A8B8D8] transition-colors">
            Forgot your password?
          </Link>
        </p>
      </form>
      ...
      {/* Dev hint */}
      <p className={cn('mt-5 pt-4 border-t border-white/[0.06]', 'text-[11px] text-[#5A6A8A] text-center leading-relaxed')}>
