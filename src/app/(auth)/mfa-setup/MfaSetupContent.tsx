...
import {
  setupMfaMock as setupMfa,
  verifyMfaMock as verifyMfa,
} from '@/lib/mock'
import { cn } from '@/lib/cn'

export default function MfaSetupContent() {
  ...
  function handleOtpComplete(code: string) {
    ...
    startTransition(async () => {
      const { data, error: apiError } = await verifyMfa({
        email,
        code,
        enable: true,
      })
          ...
          <div className="rounded-[12px] bg-[#141C30] border border-white/[0.08] p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {backupCodes.map((c) => (
                <code
                  key={c}
                  className="text-[13px] font-mono text-[#E8F0FF] bg-[#0A0E1A] rounded-[8px] px-3 py-1.5 text-center"
                >
                  {c}
                </code>
              ))}
      ...
      <div className="space-y-5">
        <div className="flex gap-3 p-3.5 rounded-[10px] bg-[rgba(0,232,122,0.06)] border border-[rgba(0,232,122,0.15)]">
          ...
          <p className="text-[12px] text-[#A8B8D8] leading-relaxed">
            ...
            <strong className="text-[#E8F0FF]">
              99% less likely
            </strong>{' '}
            to be compromised. Takes 30 seconds to set up.
          </p>
        </div>
        ...
        {loadingSetup ? (
          ...
          <div className="rounded-[12px] bg-[#141C30] border border-white/[0.08] p-4 space-y-3">
            ...
            <p className="text-[12px] text-[#A8B8D8]">
              ...
              then add a new account manually.
            </p>
